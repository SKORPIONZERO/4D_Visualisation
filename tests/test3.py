...
def calculate_vertices(centre, cube_width, line_vectors, theta):
    ...
    for k in range(len(line_vectors)):
        current_vertex_1 = centre + [-350, 0, 0] + (Rx @ line_vectors[k]) * (cube_width//2)
        current_vertex_2 = centre + (Ry @ line_vectors[k]) * (cube_width//2)
        current_vertex_3 = centre + [350, 0, 0] + (Rz @ line_vectors[k]) * (cube_width//2)
